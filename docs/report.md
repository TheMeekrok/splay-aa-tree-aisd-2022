...
Чтобы выполнить $split(i, t)$, сначала производится $access(i)$, а затем в качестве результата возвращаем два дерева, образованные левым и правым сыном $t$.
...
AA-дерево решает эту проблему следующим образом: к одной вершине можно присоединить вершину *только того же уровня, только одну и только справа*. 
